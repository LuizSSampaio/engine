...
std::optional<std::shared_ptr<Scene>> Manager::LoadScene(const std::string id) {
...
Manager::GetScene(const std::string id) const {
...
std::optional<Manager::Error> Manager::UnloadScene(const std::string id) {
    ...
    if (scene->isActive) {
        LOG_CORE_ERROR("Scene({}) can't be unloaded. It's impossible to unload "
                       "an active scene.",
                       id);
